...
class App extends React.Component {
  ...
  initSocket(url) {
    this.setState({status: 'Csatlakozás...'});
    const socket = io.connect(url);
    socket.on('connect', () => {
      this.appendMessage(`Csatlakozva a következő szerverre: ${this.state.url}`);
    ...
    socket.on('login', (data) => {
      this.appendMessage(`${data.username} bejelentkezett.`);
      this.setState({users: data.users});
    });
    socket.on('typing', (data) => {
      this.setState({status: `${data.username} éppen gépel...`});
    });
    socket.on('stop-typing', () => {
      this.setState({status: ''});
    });
    socket.on('logout', (data) => {
      this.appendMessage(`${data.username} kijelentkezett.`);
      this.setState({users: data.users});
    });
    this.socket = socket;
  }
  ...
  render() {
    return (
      <div id="login-box" ref="root">
        <div>
          <h2>Login</h2>
          <input type="url" id="server urlje" ref="url" value={this.props.url} />
          <input type="text" placeholder="felhasználónév" id="username" ref="username" onKeyDown={this.onKeyDown} autofocus />
        </div>
      </div>
    );
  }
...
class UserList extends React.Component {
  render() {
    ...
    return (
      <aside>
        <h3>Csatlakozott felhasználók</h3>
        <ul id="users">{users}</ul>
        <div id="user-stats">{users.length} felhasználó online.</div>
      </aside>
    );
  }
}
  ...
  render() {
    return (
      <div className="input">
        ...
        />
        <button id="send-btn" onClick={this.onClick}>Küldés</button>
      </div>
    );
  }
}
